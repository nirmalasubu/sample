...
{
    public class AiringMessagePusher : IAiringMessagePusherQueueApi, IAiringMessagePusherAiringApi, IAiringMessagePusherMessagePusher
    {
        private readonly MongoCollection<HistoricalMessage> _currentAirings;
        private readonly MongoCollection<HistoricalMessage> _deletedAirings;

        public AiringMessagePusher(IODTDatastore connection)
        {
            var database = connection.GetDatabase();

            _currentAirings = database.GetCollection<HistoricalMessage>(DataStoreConfiguration.CurrentAssetsCollection);
            _deletedAirings = database.GetCollection<HistoricalMessage>(DataStoreConfiguration.DeletedAssetsCollection);
        }
