...
using OnDemandTools.DAL.Modules.User.Model;
using OnDemandTools.DAL.Modules.User.Queries;
using System;
using System.Collections.Generic;
using System.Linq;
using BLModel = OnDemandTools.Business.Modules.UserPermissions.Model;
using QueueModel = OnDemandTools.Business.Modules.Queue.Model;

namespace OnDemandTools.Business.Modules.UserPermissions
{
    public class MigrateUsersAndSystem : IMigrateUsersAndSystem
    {
        IUserPermissionService _userSvc;
        IGetUsersQuery _oldUser;
        IActiveDirectoryQuery _adQuery;
        IQueueService _queueService;

        public MigrateUsersAndSystem(IUserPermissionService userSvc, IGetUsersQuery oldUser, IActiveDirectoryQuery adQuery, IQueueService queueService)
        {
            _userSvc = userSvc;
            _oldUser = oldUser;
            _adQuery = adQuery;
        ...
        private void FillDeliveryQueue(UserIdentity user, BLModel.UserPermission newPermission, List<QueueModel.Queue> queues)
        {
            ...
            else
            {
                ...
                {
                    var claimCode = claim.ToLower().Trim();
        ...
        private void FillPortalUser(UserIdentity user, AzureAdUser adUser, BLModel.UserPermission newPermission, List<BLModel.PortalModule> modules)
        {
            ...
            newPermission.Portal.IsAdmin = user.Claims.Any(e => e.ToLower() == "admin");
        ...
        private void FillApiUser(UserIdentity user, BLModel.UserPermission newPermission)
        {
            ...
            var isAdmin = user.Claims.Any(e => e.ToLower() == "admin");

            newPermission.Api.BrandPermitAll = isAdmin;
            newPermission.Api.DestinationPermitAll = isAdmin;

            var claims = new List<string>();
            if (user.Claims.Any(e => e.ToLower() == "get"))
            {
                claims.Add("get");
            }

            if (user.Claims.Any(e => e.ToLower() == "post"))
            {
                claims.Add("post");
            }

            if (user.Claims.Any(e => e.ToLower() == "delete"))
            {
                claims.Add("delete");
            }
